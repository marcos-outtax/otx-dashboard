...
export default async function handler(req, res) {
  ...
  try {
    ...
    const eventos = (data.items || []).map(ev => {
      ...
      // Extrai email do cliente — exclui emails internos da Outtax e o próprio usuário
      const emailsInternos = ['midias.outtax@gmail.com','maiccon@outtax.com.br','marcos@outtax.com.br','outtax.com.br'];
      const attendees = ev.attendees || [];
      const convidadoCliente = attendees.find(a =>
        !a.self &&
        !emailsInternos.some(ei => a.email?.includes(ei))
      );
      // Fallback: qualquer não-self se não encontrar fora dos internos
