...
module.exports = async function handler(req, res) {
  ...
  if (error) {
    return res.status(400).send(`<html><body style="font-family:sans-serif;padding:40px;background:#f0f4fb;">
      <h2 style="color:#E24B4A;">❌ Erro na autorização</h2><p>${error}</p>
      <a href="/">← Voltar ao dashboard</a></body></html>`);
  }

  if (!code) {
    return res.status(400).send(`<html><body style="font-family:sans-serif;padding:40px;background:#f0f4fb;">
      <h2 style="color:#E24B4A;">❌ Código não recebido</h2>
      <a href="/">← Voltar ao dashboard</a></body></html>`);
  }
  ...
  try {
    ...
    return res.status(200).send(`
      ...
      <style>body{font-family:sans-serif;padding:40px;background:#f0f4fb;color:#152c6b;}
      ...
      <div class="warn">⚠️ <strong>Copie os tokens abaixo e salve no Vercel como variáveis de ambiente.</strong></div>
      <div class="box"><div class="label">RDCRM_ACCESS_TOKEN</div><div class="token">${data.access_token}</div></div>
      <div class="box"><div class="label">RDCRM_REFRESH_TOKEN</div><div class="token">${data.refresh_token || '(não retornado)'}</div></div>
