export default async function handler(req, res) {
  ...
  try {
    ...
    (campsData.data || []).forEach(c => {
      const orcDiario = c.daily_budget ? parseFloat(c.daily_budget) / 100 : null;
      const orcTotal = c.lifetime_budget ? parseFloat(c.lifetime_budget) / 100 : null;
      campanhasPorId[c.id] = {
        id: c.id, nome: c.name,
    ...
    if (campanhasSemOrcamento.length > 0) {
      const adsetPromises = campanhasSemOrcamento.map(campId =>
        fetch(`https://graph.facebook.com/v19.0/${campId}/adsets?fields=daily_budget,lifetime_budget,status&limit=10&access_token=${token}`)
          .then(r => r.json())
      ...
      adsetResults.forEach(({ campId, adsets }) => {
        ...
        // Soma orçamentos de todos os ad sets ativos da campanha
        let totalDiario = 0, totalVitalicio = 0;
        adsets.forEach(as => {
          if (as.daily_budget) totalDiario += parseFloat(as.daily_budget) / 100;
          if (as.lifetime_budget) totalVitalicio += parseFloat(as.lifetime_budget) / 100;
        });
        if (campanhasPorId[campId]) {
          campanhasPorId[campId].orcamentoDiario = totalDiario > 0 ? totalDiario : null;
          campanhasPorId[campId].orcamentoTotal = totalVitalicio > 0 ? totalVitalicio : null;
