export default async function handler(req, res) {
  ...
  try {
    // 1. Busca campanhas com status e orçamento
    ...
    const campanhasSemOrcamento = [];
    (campsData.data || []).forEach(c => {
      // daily_budget e lifetime_budget vêm em centavos
      // Nunca coexistem — ou é diário ou é vitalício
      const orcDiario = (c.daily_budget && parseInt(c.daily_budget) > 0)
        ? parseInt(c.daily_budget) / 100 : null;
      const orcTotal = (c.lifetime_budget && parseInt(c.lifetime_budget) > 0 && !orcDiario)
        ? parseInt(c.lifetime_budget) / 100 : null;

      campanhasPorId[c.id] = {
        id: c.id, nome: c.name,
        status: c.effective_status || c.status,
        orcamentoDiario: orcDiario,
        orcamentoTotal: orcTotal,
        inicio: c.start_time ? c.start_time.split('T')[0] : null,
        fim: c.stop_time ? c.stop_time.split('T')[0] : null,
      };
      if (!orcDiario && !orcTotal) campanhasSemOrcamento.push(c.id);
    });

    // 2. Para campanhas sem orçamento na campanha, busca nos Ad Sets
    if (campanhasSemOrcamento.length > 0) {
      const adsetPromises = campanhasSemOrcamento.map(campId =>
        fetch(`https://graph.facebook.com/v19.0/${campId}/adsets?fields=id,name,daily_budget,lifetime_budget,bid_strategy,status&limit=20&access_token=${token}`)
          .then(r => r.json())
          .then(d => ({ campId, adsets: d.data || [] }))
          .catch(() => ({ campId, adsets: [] }))
      );
      const adsetResults = await Promise.all(adsetPromises);
      adsetResults.forEach(({ campId, adsets }) => {
        if (!adsets.length) return;
        // Separa ad sets com orçamento diário e total — não mistura os dois
        const comDiario = adsets.filter(as => as.daily_budget && parseInt(as.daily_budget) > 0);
        const comTotal = adsets.filter(as => as.lifetime_budget && parseInt(as.lifetime_budget) > 0 && !as.daily_budget);

        let orcDiario = null, orcTotal = null;

        if (comDiario.length > 0) {
          // Soma apenas os daily_budget reais (em centavos → reais)
          orcDiario = comDiario.reduce((sum, as) => sum + parseInt(as.daily_budget), 0) / 100;
        }
        if (comTotal.length > 0) {
          orcTotal = comTotal.reduce((sum, as) => sum + parseInt(as.lifetime_budget), 0) / 100;
        }

        if (campanhasPorId[campId]) {
          campanhasPorId[campId].orcamentoDiario = orcDiario;
          campanhasPorId[campId].orcamentoTotal = orcTotal;
          campanhasPorId[campId].orcamentoViaAdSet = true;
        }
      });
    }

    // 3. Busca insights com paginação
    ...
    // 4. Processa insights cruzando por campaign_id
    ...
    const campanhas = todosInsights.map(c => {
      ...
      // Orçamento: preferir diário, senão total, senão nulo
      const orcamento = info.orcamentoDiario
        ? { valor: info.orcamentoDiario, tipo: 'diário', viaAdSet: info.orcamentoViaAdSet || false }
        : info.orcamentoTotal
        ? { valor: info.orcamentoTotal, tipo: 'total', viaAdSet: info.orcamentoViaAdSet || false }
        : null;
    ...
    // 5. Campanhas sem dados no período
    Object.values(campanhasPorId).forEach(info => {
      if (!idsComInsights.has(info.id)) {
        ...
        campanhas.push({
          ...
          orcamento, inicio: info.inicio, fim: info.fim,
