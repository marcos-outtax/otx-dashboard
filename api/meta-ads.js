export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Busca insights por campanha no período — inclui campaign_id para cruzamento seguro
    const insightsFields = 'campaign_id,campaign_name,impressions,clicks,spend,reach,cpc,ctr,actions';
    const insightsUrl = `https://graph.facebook.com/v19.0/${actId}/insights?fields=${encodeURIComponent(insightsFields)}&time_range=${encodeURIComponent(timeRange)}&level=campaign&limit=200&access_token=${token}`;
