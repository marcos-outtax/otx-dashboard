...
/* ── LIGHTBOX CRIATIVO ── */
.lightbox-ov{display:none;position:fixed;inset:0;background:rgba(0,0,0,.82);z-index:9999;align-items:center;justify-content:center;padding:20px;}
.lightbox-ov.open{display:flex;}
.lightbox-box{background:#1a1a2e;border-radius:16px;max-width:600px;width:100%;max-height:92vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,.6);}
.lightbox-img{width:100%;height:320px;display:block;object-fit:contain;border-radius:16px 16px 0 0;background:#111;}
.lightbox-body{padding:18px 20px;}
.lightbox-status{display:inline-block;font-size:10px;padding:2px 10px;border-radius:20px;font-weight:700;margin-bottom:10px;}
.lightbox-titulo{font-size:15px;font-weight:700;color:#fff;margin-bottom:6px;line-height:1.4;}
.lightbox-corpo{font-size:12px;color:rgba(255,255,255,.65);line-height:1.7;margin-bottom:14px;}
.lightbox-metricas{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:14px;}
.lightbox-met{background:rgba(255,255,255,.07);border-radius:8px;padding:8px 10px;}
.lightbox-met .lm-l{font-size:9px;color:rgba(255,255,255,.45);text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px;}
.lightbox-met .lm-v{font-size:14px;font-weight:700;color:#fff;}
.lightbox-close{background:rgba(255,255,255,.12);border:none;color:#fff;font-size:18px;cursor:pointer;border-radius:8px;padding:6px 14px;float:right;margin:12px 12px 0 0;}
.lightbox-close:hover{background:rgba(255,255,255,.22);}
/* ── FILTRO CRIATIVOS ── */
.ad-filtro{display:flex;gap:6px;margin-bottom:10px;}
.ad-filtro-btn{padding:4px 12px;font-size:11px;font-weight:600;border-radius:20px;border:1px solid var(--border);background:var(--branco);color:var(--text-muted);cursor:pointer;transition:all .15s;}
.ad-filtro-btn.ativo{background:var(--azul-escuro);color:#fff;border-color:var(--azul-escuro);}
.ad-filtro-btn:hover:not(.ativo){background:var(--azul-pale);border-color:var(--azul-medio);}
</style>
</head>
<body>
...
<div class="main-content">
  ...
  <div class="page" id="page-fbmkt">
    ...
    <!-- FILTROS UNIFICADOS -->
    <div style="background:var(--branco);border:1px solid var(--border);border-radius:12px;padding:14px 16px;margin-bottom:16px;">
...
<!-- LIGHTBOX CRIATIVO -->
<div class="lightbox-ov" id="lightbox-ov" onclick="fecharLightbox(event)">
  <div class="lightbox-box" onclick="event.stopPropagation()">
    <button class="lightbox-close" onclick="fecharLightbox()">✕ Fechar</button>
    <div style="background:#111;border-radius:16px 16px 0 0;overflow:hidden;">
      <img id="lightbox-img" src="" alt="Criativo" class="lightbox-img" style="display:none;" onerror="this.style.display='none';document.getElementById('lightbox-no-img').style.display='flex'">
      <div id="lightbox-no-img" style="display:none;height:320px;align-items:center;justify-content:center;font-size:48px;">🖼️</div>
    </div>
    <div class="lightbox-body">
      <div id="lightbox-status" class="lightbox-status"></div>
      <div id="lightbox-titulo" class="lightbox-titulo"></div>
      <div id="lightbox-corpo" class="lightbox-corpo"></div>
      <div class="lightbox-metricas">
        <div class="lightbox-met"><div class="lm-l">Investimento</div><div class="lm-v" id="lb-invest">—</div></div>
        <div class="lightbox-met"><div class="lm-l">Leads</div><div class="lm-v" id="lb-leads">—</div></div>
        <div class="lightbox-met"><div class="lm-l">CPL</div><div class="lm-v" id="lb-cpl">—</div></div>
        <div class="lightbox-met"><div class="lm-l">CTR</div><div class="lm-v" id="lb-ctr">—</div></div>
        <div class="lightbox-met"><div class="lm-l">Alcance</div><div class="lm-v" id="lb-alcance">—</div></div>
        <div class="lightbox-met"><div class="lm-l">Impressões</div><div class="lm-v" id="lb-impressoes">—</div></div>
        <div class="lightbox-met"><div class="lm-l">CPC</div><div class="lm-v" id="lb-cpc">—</div></div>
        <div class="lightbox-met"><div class="lm-l">Conv.</div><div class="lm-v" id="lb-conv">—</div></div>
      </div>
    </div>
  </div>
</div>
...
// ── DELEGAÇÃO DE EVENTOS — FILTRO CRIATIVOS + TOGGLE ─────
document.addEventListener('click',function(e){
  // Filtro ativos/pausados/todos
  const btn=e.target.closest('.ad-filtro-btn');
  if(btn){
    const filtro=btn.getAttribute('data-filtro');
    const bodyId=btn.getAttribute('data-body');
    if(filtro&&bodyId)filtrarAds(bodyId,filtro,btn);
    return;
  }
  // Toggle abrir/fechar seção de criativos
  const hdr=e.target.closest('.ad-sec-hdr');
  if(hdr){
    const targetId=hdr.getAttribute('data-target');
    if(targetId)toggleAdBody(targetId);
    return;
  }
});

// ── LIGHTBOX CRIATIVO ────────────────────────────────────
function abrirLightbox(ad){
  ...
  if(ad.criativo&&ad.criativo.thumbnail){img.src=ad.criativo.thumbnail;img.style.display='block';noImg.style.display='none';}
  else{img.style.display='none';noImg.style.display='flex';}
  const st=document.getElementById('lightbox-status');
  st.textContent=isAtivo?'✅ Ativo':'⏸ Pausado';
  st.style.cssText=isAtivo?'background:#EAF3DE;color:#27500A;display:inline-block;font-size:10px;padding:2px 10px;border-radius:20px;font-weight:700;margin-bottom:10px;':'background:#f0f0f0;color:#9e9e9e;display:inline-block;font-size:10px;padding:2px 10px;border-radius:20px;font-weight:700;margin-bottom:10px;';
  document.getElementById('lightbox-titulo').textContent=(ad.criativo&&ad.criativo.titulo)||ad.nome||'Anúncio';
  document.getElementById('lightbox-corpo').textContent=(ad.criativo&&ad.criativo.corpo)||'';
  document.getElementById('lb-invest').textContent=fmtBRL(ad.investimento);
  document.getElementById('lb-leads').textContent=ad.leads||'0';
  document.getElementById('lb-cpl').textContent=ad.cpl?fmtBRL(ad.cpl):'-';
  document.getElementById('lb-ctr').textContent=ad.ctr?ad.ctr+'%':'-';
  document.getElementById('lb-alcance').textContent=fmtN(ad.alcance);
  document.getElementById('lb-impressoes').textContent=fmtN(ad.impressoes);
  document.getElementById('lb-cpc').textContent=ad.cpc?fmtBRL(ad.cpc):'-';
  document.getElementById('lb-conv').textContent=ad.taxaConversao?ad.taxaConversao+'%':'-';
  document.getElementById('lightbox-ov').classList.add('open');
}
function fecharLightbox(e){
  if(e&&e.target!==document.getElementById('lightbox-ov'))return;
  document.getElementById('lightbox-ov').classList.remove('open');
}
function abrirLightboxKey(key){
  const ad=window['_adStore_'+key];
  if(ad)abrirLightbox(ad);
}

// ── FILTRO CRIATIVOS ─────────────────────────────────────
function filtrarAds(bodyId,filtro,btnEl){
  const wrap=document.getElementById('filtro-wrap-'+bodyId);
  if(wrap)wrap.querySelectorAll('.ad-filtro-btn').forEach(function(b){b.classList.remove('ativo');});
  if(btnEl)btnEl.classList.add('ativo');
  const body=document.getElementById(bodyId);
  if(!body)return;
  body.querySelectorAll('.ad-card-item').forEach(function(el){
    const status=el.getAttribute('data-status');
    if(filtro==='todos')el.style.display='';
    else if(filtro==='ativos')el.style.display=(status==='ACTIVE')?'':'none';
    else el.style.display=(status!=='ACTIVE')?'':'none';
  });
}
function toggleAdBody(id){
  const b=document.getElementById(id);
  if(!b)return;
  const isOpen=b.style.display!=='none';
  b.style.display=isOpen?'none':'block';
  const wrap=b.closest('.ad-sec-wrap');
  if(wrap){
    const chev=wrap.querySelector('.ad-chev');
    if(chev)chev.style.transform=isOpen?'':'rotate(180deg)';
  }
}

// ── CARD AD ──────────────────────────────────────────────
let _adBodyCounter=0;
function cardAd(ad){
  const isAtivo=ad.status==='ACTIVE';
  ...
  const adKey='ad'+Math.random().toString(36).slice(2,9);
  window['_adStore_'+adKey]=ad;
  const thumb=(ad.criativo&&ad.criativo.thumbnail)
    ?('<img src="'+ad.criativo.thumbnail+'" style="width:56px;height:56px;object-fit:contain;border-radius:8px;flex-shrink:0;border:1px solid var(--border);cursor:pointer;transition:transform .15s;background:#f0f4fb;" onmouseover="this.style.transform=\'scale(1.07)\'" onmouseout="this.style.transform=\'\'" onclick="abrirLightboxKey(\''+adKey+'\')" onerror="this.style.display=\'none\'">')
    :('<div onclick="abrirLightboxKey(\''+adKey+'\')" style="width:56px;height:56px;border-radius:8px;background:var(--azul-pale);display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;cursor:pointer;">🖼️</div>');
  const titulo=(ad.criativo&&ad.criativo.titulo)||ad.nome||'Anúncio';
  const corpo=(ad.criativo&&ad.criativo.corpo)?('<div style="font-size:10px;color:var(--text-muted);margin-top:2px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">'+ad.criativo.corpo+'</div>'):'';
  return '<div style="display:flex;gap:10px;align-items:flex-start;padding:10px 12px;border:1px solid var(--border);border-radius:8px;margin-bottom:6px;background:var(--branco);">'
    ...
    +'<span style="font-size:11px;font-weight:600;color:var(--azul-escuro);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:260px;">'+titulo+'</span>'
    ...
    +'<div style="background:var(--azul-pale);border-radius:6px;padding:5px 7px;"><div style="font-size:9px;color:var(--text-muted);margin-bottom:1px;">Investimento</div><div style="font-size:11px;font-weight:700;color:var(--azul-escuro);">'+fmtBRL(ad.investimento)+'</div></div>'
    +'<div style="background:var(--azul-pale);border-radius:6px;padding:5px 7px;"><div style="font-size:9px;color:var(--text-muted);margin-bottom:1px;">Leads</div><div style="font-size:11px;font-weight:700;color:'+(ad.leads>0?'#1D9E75':'var(--azul-escuro)')+';">'+(ad.leads||'0')+'</div></div>'
    +'<div style="background:var(--azul-pale);border-radius:6px;padding:5px 7px;"><div style="font-size:9px;color:var(--text-muted);margin-bottom:1px;">CPL</div><div style="font-size:11px;font-weight:700;color:var(--azul-escuro);">'+(ad.cpl?fmtBRL(ad.cpl):'-')+'</div></div>'
    +'<div style="background:var(--azul-pale);border-radius:6px;padding:5px 7px;"><div style="font-size:9px;color:var(--text-muted);margin-bottom:1px;">CTR</div><div style="font-size:11px;font-weight:700;color:var(--azul-escuro);">'+(ad.ctr?ad.ctr+'%':'-')+'</div></div>'
    +'</div>'
    +'</div>'
    +'</div>';
}
...
function secaoCreativos(ads){
  ...
  const bodyId='adsbody'+(++_adBodyCounter);
  const todos=[...ativas,...pausadas];
  const htmlCards=todos.map(function(a){
    return '<div class="ad-card-item" data-status="'+a.status+'">'+cardAd(a)+'</div>';
  }).join('');
  return '<div class="ad-sec-wrap" style="border-top:1px solid var(--border);margin-top:12px;padding-top:12px;">'
    +'<div class="ad-sec-hdr" data-target="'+bodyId+'" style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;margin-bottom:8px;padding:10px 14px;background:var(--azul-pale);border-radius:8px;">'
    +'<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">'
    +'<span style="font-size:13px;font-weight:700;color:var(--azul-escuro);">🎨 Criativos</span>'
    +'<span style="font-size:11px;color:var(--text-muted);font-weight:400;">'+totalAds+' total</span>'
    +'<span style="font-size:11px;font-weight:700;color:#1D9E75;background:#EAF3DE;padding:3px 10px;border-radius:20px;">✅ '+ativas.length+' ativos</span>'
    +'<span style="font-size:11px;font-weight:700;color:#6b6b6b;background:#f0f0f0;padding:3px 10px;border-radius:20px;">⏸ '+pausadas.length+' pausados</span>'
    +'</div>'
    +'<span class="ad-chev" style="font-size:10px;color:var(--text-muted);transition:transform .2s;transform:rotate(180deg);">▼</span>'
    +'</div>'
    +'<div id="'+bodyId+'" style="display:block;padding-top:8px;">'
    +'<div class="ad-filtro" id="filtro-wrap-'+bodyId+'" style="margin-bottom:12px;">'
    +'<button class="ad-filtro-btn ativo" data-filtro="todos" data-body="'+bodyId+'">Todos ('+totalAds+')</button>'
    +'<button class="ad-filtro-btn" data-filtro="ativos" data-body="'+bodyId+'">✅ Ativos ('+ativas.length+')</button>'
    +'<button class="ad-filtro-btn" data-filtro="pausados" data-body="'+bodyId+'">⏸ Pausados ('+pausadas.length+')</button>'
    +'</div>'
    +htmlCards
    +'</div>'
    +'</div>';
...
function cardFacebook(c,de,ate){
  ...
  const adsAtivos=(c.ads&&c.ads.ativas)?c.ads.ativas.length:0;
  const adsPausados=(c.ads&&c.ads.pausadas)?c.ads.pausadas.length:0;
  const totalAds=adsAtivos+adsPausados;
  const adsBadge=totalAds>0
    ?('<span style="font-size:10px;color:var(--text-muted);">🎨 '+totalAds+' criativo'+(totalAds!==1?'s':'')+'</span>'
      +'<span style="font-size:10px;font-weight:600;color:#1D9E75;background:#EAF3DE;padding:1px 7px;border-radius:20px;">✅ '+adsAtivos+'</span>'
      +'<span style="font-size:10px;font-weight:600;color:#6b6b6b;background:#f0f0f0;padding:1px 7px;border-radius:20px;">⏸ '+adsPausados+'</span>')
    :'';
  return '<div class="card" style="border-left:4px solid '+cardBorder+';margin-bottom:8px;">'
    ...
    +'<div class="card-sub" style="margin-top:3px;display:flex;gap:6px;flex-wrap:wrap;align-items:center;">'
    ...
    +semDados
    +'<div class="three-col"><div class="ib"><div class="l">Investimento</div><div class="v">'+fmtBRL(c.investimento)+'</div></div><div class="ib"><div class="l">Leads gerados</div><div class="v">'+(c.leads||'0')+'</div></div><div class="ib"><div class="l">CPL</div><div class="v">'+fmtBRL(c.cpl)+'</div></div></div>'
    +'<div class="three-col"><div class="ib"><div class="l">Alcance</div><div class="v">'+fmtNum(c.alcance)+'</div></div><div class="ib"><div class="l">Impressões</div><div class="v">'+fmtNum(c.impressoes)+'</div></div><div class="ib"><div class="l">Cliques</div><div class="v">'+fmtNum(c.cliques)+'</div></div></div>'
    +'<div class="three-col"><div class="ib"><div class="l">CTR</div><div class="v">'+(c.ctr?c.ctr+'%':'-')+'</div></div><div class="ib"><div class="l">CPC médio</div><div class="v">'+fmtBRL(c.cpc)+'</div></div><div class="ib"><div class="l">Taxa de conversão</div><div class="v">'+(c.taxaConversao?c.taxaConversao+'%':'-')+'</div></div></div>'
    +'<div class="two-col"><div class="ib"><div class="l">Orçamento</div><div class="v">'+orcamentoLabel+'</div></div><div class="ib"><div class="l">Período da campanha</div><div class="v">'+periodo+'</div></div></div>'
