export default async function handler(req, res) {
  ...
  const sessionToken = 'otx-' + Buffer.from(user.usuario + user.senha).toString('hex').substring(0, 32);
  const csrfToken = 'csrf-' + Buffer.from(sessionToken + 'csrf').toString('hex').substring(0, 24);
